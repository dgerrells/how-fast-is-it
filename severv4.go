...
// 	clientSendChannels[idx] = make(chan []byte, 1)
// 	clientsMu.Unlock()

// 	log.Printf("Client connected\n")

// 	go writePump(conn, clientSendChannels[idx])
...
// 		}
// 		clientsMu.Unlock()
// 		close(clientSendChannels[idx])
// 		clientSendChannels[idx] = nil
// 		conn.Close()
...
// 			select {
// 			case clientSendChannels[i] <- dataToSend:
// 				// Message was sent to the channel
// 			default:
// 				// Channel is full, meaning the client is too slow. Drop the frame and
// 				// flag a full frame resend for the next frame.
...
// func writePump(conn *websocket.Conn, sendChannel <-chan []byte) {
// 	for {
// 		message, ok := <-sendChannel
// 		if !ok {
// 			return
// 		}
...
// 		count := 1
// 		j := i + 1
// 		for j < len(data) && data[j] == currentByte && count < 255 {
// 			count++
// 			j++
// 		}
...
// 		deltaBytes := CreateDeltaBuffer(lastFrameBuffer, framebuffer)
// 		rleBytes := RLEncodeV2(deltaBytes)
// 		copy(lastFrameBuffer, framebuffer)
...
// func WriteVarint4(buf *bytes.Buffer, x uint32) {
// 	for x >= 0x10 {
// 		buf.WriteByte(byte(x&0x0F) | 0x10)
// 		x >>= 4
// 	}
// 	buf.WriteByte(byte(x & 0x0F))
// }

// func RLEncodeV2(data []byte) []byte {
...
// 		for j < len(data) && data[j] == currentByte {
// 			count++
// 			j++
// 		}

// 		if count > 1 || currentByte == sentinel {
// 			encoded.WriteByte(sentinel)
// 			WriteVarint4(&encoded, uint32(count))
